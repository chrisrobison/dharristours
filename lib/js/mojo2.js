...
function mojo2(data) {
   if (data) {
      ...
      if (s["Component"]) {
         for (tabidx in s["Component"]) {
            
            item = s["Component"][tabidx];
            tab = s["Component"][tabidx]["Component"];
...
function mojo_init() {
   ...
   $("#tabs .tab a").click(function(event) {
      ...
      $(".active").css({"z-index":"0"}).animate({"top":"220px","opacity":"0"}, 400, function() { $(this).removeClass('active').hide(); });  //Remove any "active" class
   ...
   $("#theme").change(function() {
      ...
      setTimeout(function() { $.getJSON("/clients/mediaplex/genad.php?out=json", function(data) { mojoAd = mojo2(data); }); }, 500);
      $("body").data("dest", $(this).val());
      setTimeout(function() { document.location = $("body").data("dest"); }, 800);
   });
}
