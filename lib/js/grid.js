...
$(document).ready(function($) {
   ...
   */
   ...
   if (grid && !window._NOGRID) {
      setGridHeight();

      grid.jqGrid('setGridParam', {   
         beforeSelectRow: function(id, e) {
            if (simpleConfig.modified && !confirm("There are changes that have *NOT* been saved!\nClick OK to discard your modifications   -or-\nClick Cancel to go back and Save your changes.")) {
               e.preventDefault();
               e.stopPropagation();
               return false;
            } else {
      ...
      setGridHeight();
      if (!simpleConfig._cache) simpleConfig._cache = {};
   ...
   $("form#simpleForm").on("input", ":input", function(e) { doModify($(this)); });
...
function fillForm(rsc, id, data, prefx) {
   ...
   if (data) {
                     ...
                     try {
                        ...
                        if (($this[0].nodeName === "SELECT") || ($this[0].nodeName === "INPUT") || ($this[0].nodeName==="TEXTAREA")) {
                           ...
                           if ($this.is('input[type="checkbox"]')) {
                              if (data[key] && data[key] != "false" && data[key] != "0" && data[key] != "No" && data[key] != "no") {
                              ...
                              // console.log("*** change event fired for "+key+"  ***");
                           }
                           // $this.change();
      ...
      if (window.loadFormCallback) {
         window.loadFormCallback(data);
      }
   }
...
function populateData(data, rsc, id, callback) {
      var $tmpel, related;
      simpleConfig.record = simpleConfig.current = data;
      if (!simpleConfig._cache[rsc]) simpleConfig._cache[rsc] = [];
      simpleConfig._cache[rsc][id] = data;
      ...
      setTimeout(function() { showLoading(false); }, 1000);
...
function loadRecord(rsc, id, callback) {
   ...
   if (!simpleConfig._cache[rsc][id]) {
      $.getJSON("ctl.php", {"x":"related", "rsc":rsc, "id":id}, function(data) {
         populateData(data, rsc, id, callback);
      });
   } else {
         populateData(simpleConfig._cache[rsc][id], rsc, id, callback);
   }
}
...
//cdr sez: it's renaming them to ensure proper ID's for each form element
      allInputs.each(function(idx) {
         ...
         if (newname) {
            ...
            if ((rel=="data") || ((rel!="data") && (typ!='hidden'))) {
               if (newname.match(/\[.*?\]/)) {
                  // The next two lines were commented out but are needed
                  // to ensure the correct ID is set for each form element
                  newname = newname.replace(/\[.*?\]/, "["+sid+"]");
                  $(this).attr('name', newname);
             
                  // This conditional breaks things. These field names are specific to dharristours
                  // and should not be in the global library.  It is fine to double check for 
                  // the correct ID but not for the specific fields.
                  // 
   ...
   $(".formHeading").css({"background":"#c0c0c0","color":"#000"});
   setTimeout(function() { showLoading(false); }, 1000);
   return false;
}
...
function postData(url, data, callback) {
   // Only POST data one transaction at a time, deferring if busy
   // Check for value in 'simpleConfig.working' attribute 
   if (!simpleConfig.working) {
      simpleConfig.working = true;
      ...
      console.log("Went to postData but it was busy. Waiting 500ms...");
      setTimeout(function() { postData(url, data, post); }, 500);
   }
...
function doCopy(parentID) {
   ...
   // This grabs all input form elements EXCEPT hidden ones that don't have a rel=data attribute
 ...
 //pepe
      ...
      if ((rel=="data") || ((rel!="data") && (typ!='hidden'))) {
         ...
         if (newname) newname = newname.replace(/\[.*?\]/, "[new1]");
         me.attr('name', newname);
         simpleConfig.record[key] = (key == "LastModifiedBy") ? simpleConfig.userEmail : me.val();
     } 
 //end pepe
      newname = $(this).attr('name').replace(/\[.*?\]/, "[new1]"); //pepe commented, cdr uncommented 2024-05-07
      $(this).attr('name', newname); //pepe commented, cdr uncommented 2024-05-07
   });
    doModify(); //pepe commented, cdr uncommented
...
function toggleGrid(setstate) {
   ...
   function setGridHeight(sz, anim) {
      ...
      if (anim) {
         $("#gview_mygrid").animate({ height: gridH + 50}, 250, "swing");
         $("#gbox_mygrid").animate({ height: gridH + 70 }, 250, "swing", function() {
            $("#mygrid").setGridHeight(gridH);
            doLayout();
            console.log("[setGridHeight] With animation | Grid height: " + gridH);
         });
      } else {
         $("#gview_mygrid").css({ height: gridH + 50});
         $("#gbox_mygrid").css({ height: gridH + 70 });
