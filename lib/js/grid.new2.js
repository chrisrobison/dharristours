...
$(document).ready(function($) {
   ...
   $("#ActionsButton").click(function() { $("#ActionsMenu").slideToggle(100); });
   for (var a in actions) {
      $("#"+a+"Button").click(function(event) { 
         $("#ActionsMenu").slideUp(100); 
   ...
   grid.jqGrid('setGridParam', {   
         onSelectRow: function(id) { selectRow(id); },
   ...
   $("#relatedData").on("click", "h3", function() { $(this).toggleClass('open'); });
...
function saveLayout(grid) {
   ...
   var    config = $(grid).jqGridExport({"exptype":"jsonstring"}), 
       oldconfig = JSON.parse(localStorage[simpleConfig.resource + simpleConfig.pid]),
         colConf = { "columnOrder": oldconfig['columnOrder'], "columnDisplay": oldconfig['columnDisplay'], "config": config };
   localStorage.setItem(simpleConfig.resource + simpleConfig.pid, JSON.stringify(colConf));

   postData("ctl.php", {"x":"storeConfig", "data":colConf, "mid": simpleConfig.ModelID, "pid": simpleConfig.pid, "rsc": simpleConfig.resource});
}
