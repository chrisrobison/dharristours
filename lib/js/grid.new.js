...
$(document).ready(function($) {
   ...
   var actions = {
      ...
      "Copy":        function() {$("#ActionsMenu").slideUp(200);return doCopy();},
   ...
   $("#ActionsButton").click(function() { $("#ActionsMenu").slideToggle(200); });
   for (var a in actions) {
      $("#"+a+"Button").click(function(event) { 
         $("#ActionsMenu").slideUp(200); 
  ...
  //$("#gbox_mygrid").height(gridH);
   grid.jqGrid("gridResize", { minWidth:200,maxWidth:3500,minHeight:80,maxHeight:2000,stop:function() { doLayout('open'); } });
   grid.jqGrid("resizeStop", function(width, index) { 
         alert('resize column ' + index + ' to ' + width + ' pixels'); 
      }
   );
   grid.jqGrid({gridComplete: function() { $("#mygrid").setGridWidth($("body").width() - 2 ); }});
   ...
   if (localStorage[simpleConfig.resource + simpleConfig.pid]) {
      var colConf = JSON.parse(localStorage[simpleConfig.resource + simpleConfig.pid]);
   ...
   $("form#simpleForm :input").live("change", function(e) { doModify($(this)); });
 });
...
function doNew() {
   ...
   $("a.simpleButton", $("#toolbar")).show(200);
...
*/
   window.open("/print.php?pid="+simpleConfig.pid+"&id="+simpleConfig.id);
}
