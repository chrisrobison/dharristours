...
function handlePostData(&$data) {
	...
	if(getPost("tidy") && getPost("tidy") != getConfigValue("tidy")) {
		// CLEAN PATH
		$tidy = getPost("tidy");
		if(substr($tidy, -1) != "/") {
			$tidy .= "/";
		...
		if(!checkTidy($tidy)) {
			$data["error"][] = "CSS Tidy was not found at the given location!";
			$tidy = "";
		}else{
			$data["success"][] = "CSS Tidy is now enabled!";
		}
		
		setConfigValue("tidy", $tidy);
	}
...
function checkTidy($path) {
	if(file_exists($path."class.csstidy.php")) {
		return true;
	}else{
		return false;
	}
...
function onActionSaveRoutine() {
	...
	// TIDY IF ENABLED
	if(getConfigValue("tidy")) {
		$stylesheet = tidyCss($stylesheet, getConfigValue("tidy"));
	}
...
function tidyCss($contents, $path) {
	require_once($path.'class.csstidy.php');
...
function printTemplate($data) { ?>
	<html>
		<head>
			...
			<?php if($data["user_set"] && $data["pass_set"] && $data["logged_in"]) { ?>
			...
			<form id="firefile-form" method="POST">
				<?php if($data["user_set"] || $data["pass_set"]){ ?>
					<div id="login-panel">
						<div class="logo-float">
							<img src="http://www.strebitzer.at/projects/firefile/firefile_32x32.png" width="32" height="32" alt="FireFile" title="FireFile" />
				...
				<?php if($data["logged_in"]) { ?>
					<div class="config-pane">
						...
						<h1>CHANGE PATH TO CSS TIDY (leave blank to disable):</h1>
						<div class="config-pane">
							<label for="tidy">CSS TIDY PATH:</label>
							<input type="text" value="<?php echo getConfigValue("tidy"); ?>" id="tidy" name="tidy" />
							<input class="inner_submit" type="submit" value="APPLY" />
						</div>
						<h1>FIREFILE INFORMATION:</h1>
						<div class="config-pane">
							<label for="install">INSTALL FIREFILE BROWSER ADDON:</label>
							<label> (YOU HAVE TO OPEN THE FILE WITH FIREFOX)</label>
							<span id="install" class="firefile-install"><a href="http://www.strebitzer.at/projects/firefile/firefile.xpi" target="_self">INSTALL</a></span>
						</div>
						<?php if($data["user_set"] && $data["pass_set"]) { ?>
							<div class="config-pane">
