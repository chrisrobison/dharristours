<?php
   ...
   if ($in['x']) {
      switch($in['x']) {
         case "get":
            ...
            break;
         case "store":
            $out = saveDVIR($link, $in);
            break;
         case "update":
            $out = updateDVIR($link, $in);
            break;
         case "list":
            $out = listDVIR($link, $in);
            break;
      }
   ...
   function listDVIR($link, $in) {
      $out = array(); $cnt = 0;
   
      if ($in['date']) {
         $now = date("Y-m-d", strtotime($in['date']));
      } else {
         $now = date("Y-m-d");
      }
...
//       $in = $_REQUEST;
      ...
      $sql = "SELECT * FROM DVIR where Date>='{$first}' AND JobDate<='{$last}'";
   ...
   function getDVIR($link, $id) {
      $sql = "SELECT * from DVIR where DVIRID='".$id."'";
