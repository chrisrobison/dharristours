...
list($output, $jstree) = dump($obj, 1, 1);
   ...
   foreach ($obj as $key=>$val) {
      ...
      if ((is_object($val)) || (is_array($val))) {
         if (count((array)$val)) {
            ...
            $html .= "<div id='node_{$parent}_$id' class='node branchClosedEnd' onclick=\"toggleTree('node_{$parent}_$id');\">".$key." <div class='val'>[$val]</div></div>\n";
         ...
         if ($val) $html .= "<div id='node_{$parent}_$id' class='node nodeClosed'>".$key." <div class='val'>[$val]</div></div>\n";
      }
   }
