...
let app = {
    ...
    */
    initAuthentication: function(onAuthSuccess) {
    ...
    initFirebase: function() {
       ...
       var fbuses = firebase.database().ref('bus/');
       fbuses.on('value', function(snapshot) {
    ...
    initMap: function() {
      ...
      trafficLayer.setMap(app.state.map);
      
      app.initAuthentication(app.initFirebase.bind(undefined, ''));
    },
    ...
    addBuses: function(buses) {
       console.dir(buses);
       for (var i = 0; i < buses.length; i++) {
          app.state.markers[i] = new google.maps.Marker({
    ...
    updateBuses: function(buses) {
       console.dir(buses);
       if (buses && buses.length) {
          for (var i = 0; i < buses.length; i++) {
             if (i < 2000) { 
                if (buses[i] && buses[i].objectno) {
                   app.state.fleet[buses[i].objectno] = buses[i];
                }
                
                if (!app.state.markers[i] && buses[i]) {
                   window.busIcon = {
       ...
       //                     icon: busIcon,
                        icon: {
                         url: app.state.iconPath + '/bus10.png',
                         labelOrigin: new google.maps.Point(60,20),
                         size: new google.maps.Size(150, 100),
                   ...
                   app.state.buses[i] = buses[i];
                   app.state.markers[i].bus = buses[i];
                   app.state.bounds.extend(app.state.markers[i].position);
                   var iw;
                   app.state.markers[i].infowin = iw = new google.maps.InfoWindow({ 
                   ...
                   let mybus = app.state.buses[i];
                   let mark = app.state.markers[i];


             fetch(url).then(res => res.json())
             .then((out) => {
                  var html = "<h2>Bus #" + mybus['objectname'];
                  if (out && out[0] && out[0].Driver) {
                      html += " - " + out[0].Driver;
                  }
                  html += "</h2>" + mybus['postext'] + "<br>\n";
                  if (out.length) {
             ...
             .catch(err => { throw err });
                   app.state.markers[i].addListener('click', function() {
                      this.infowin.open(map, this);
                   });

                } else {
                   if (app.state.markers && app.state.markers[i]) {
                      app.state.markers[i].setPosition(new google.maps.LatLng((buses[i].latitude_mdeg / 1000000), (buses[i].longitude_mdeg / 1000000)));
          ...
          if (!app.state.initfit) {
             app.state.map.fitBounds(app.state.bounds);
             app.state.initfit = 1;
          }
       } else {
          console.log("Tried to update buses but they're missing!!");
       }
       var markerCluster = new MarkerClusterer(app.state.map, app.state.markers, {imagePath: '/where/img/m', styles: [
    ...
    distanceInMilesBetweenEarthCoordinates: function(lat1, lon1, lat2, lon2) {
      var earthRadiusMiles = 3959;

      var dLat = app.degreesToRadians(lat2-lat1);
      var dLon = app.degreesToRadians(lon2-lon1);

      lat1 = app.degreesToRadians(lat1);
      lat2 = app.degreesToRadians(lat2);
