<?php
   ...
   $rsc = ($in['rsc']) ? $in['rsc'] : 'JobCal';
  
   $boss->db->addResource($rsc);
   $boss->db->{$rsc}->getlist();//gets clamps...CLLAAAMMMPPP
   $events = $boss->db->{$rsc}->{$rsc};
?>
...
/*   secondUserCalendar.addEvents(
               ...
               <?php 
                  $jsevents = Array();
                  foreach ($events as $event) {
                     // if ($event->LoginID == $_SESSION['LoginID']) {
                        $out = "{startDateTime: '".preg_replace("/\s/", "T", $event->StartDateTime)."Z',";
                        $out .= "duration: ".$event->Duration.",";
                        $out .= "summary: '".$event->Calendar."',";
                        $out .= "description: '".$event->Notes."'}";
...
//                        $out .= "Description: '".$event->Description."'}";
			...
			function addCalEvents () {
				if (firstUserCalendar) dojocal.addCalendar(firstUserCalendar);
				if (secondUserCalendar) dojocal.addCalendar(secondUserCalendar);
			}
			addCalEvents();
...
<body>
	...
	<div id="myParamsContainer" class='closed'>
	...
	<div id="handle" class="closed">|</div>

	<div class="myTitleArea">
	</div>


	<div id="cal" dojoType="dojoc.dojocal.SlickGrid" class="myDojocal closed" viewMode="WEEK"></div>
...
<script type='text/javascript'>
   $(document).ready(function() {
      $("#handle").click(function() { 
         ...
         $("#myParamsContainer").toggleClass("closed");
      });
   });
</script>
