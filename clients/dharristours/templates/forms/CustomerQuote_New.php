  ...
  function upAll(id) {
   ...
   function ToggleEndTime() {
	if ($("#RoundTrip").val()==0) {
	   ...
	   $("#SpecialInstructions").val('One Way Xfer');
	} else {
	   $("#endspan").show(2000);
	   updateTime('Start',true);
	   $("#"+"SpecialInstructions").val('TBD');
	}
   }
</script>
<div class='tableGroup'>
   ...
   <div id='emailOption' class='msg' >
      ...
      <input type='hidden' name='sendEmail' id='sendEmail' value='New' />
   </div>
      <div class='fieldcolumn fieldfloater'>
         <div class='contentField'><span class='fieldLabel'>Company Name: </span><?php $boss->db->addResource("Business");$arr = $boss->db->Business->getlist("LoginID='".$_SESSION['LoginID']."'");print $boss->utility->buildSelect($arr, $current->BusinessID, "BusinessID", "Business", "Quote[$current->QuoteID][BusinessID]");?></div>
         <div class='contentField'><label>Trip Date: </label><input type='text' dbtype='date' name='Quote[<?php print $current->QuoteID; ?>][Date]' id='Date' value='' size='25' class='boxValue date' /></div>
...
<select id='End_hour' onchange="updateTime('End');">
      ...
      <div class='contentField'><span class='fieldLabel'>Est Hours: </span><input type='text' name='Quote[<?php print $current->QuoteID; ?>][Hours]' id='Hours' value='<?php print $current->Hours; ?>' size='5' class='boxValue' /></div>
         <div class='contentField'><span class='fieldLabel'># of Passengers: </span><input type='text' name='Quote[<?php print $current->QuoteID; ?>][NumberOfPassengers]' id='NumberOfPassengers' value='<?php print $current->NumberOfPassengers; ?>' size='5' class='boxValue' /></div>
      </div>
      <div class='fieldcolumn fieldfloater'>
         ...
         <div class='contentField'><span class='fieldLabel'>Phone: </span><input type='text' name='Quote[<?php print $current->QuoteID; ?>][ContactPhone]' id='ContactPhone' value='<?php print $current->ContactPhone; ?>' size='50' class='boxValue' /></div>
         ...
         <div class='contentField'><label>Wheel Chair: </label><select dbtype='tinyint(1)' name='Quote[<?php print $current->QuoteID; ?>][WheelChair]' id='WheelChair'><option value='0'>No</option><option value='1'>Yes</option></select></div>
         <div class='contentField'><label>School Activity: </label><select dbtype='tinyint(1)' name='Quote[<?php print $current->QuoteID; ?>][SPAB]' id='SPAB'><option value='0'>No</option><option value='1'>Yes</option></select></div>
      </div>
      <div id='specspan' style="clear:left" class='contentField'><span class='fieldLabel'>Note: </span><textarea name='Quote[<?php print $current->QuoteID; ?>][SpecialInstructions]' id='SpecialInstructions' class='textBox' style='width:45em;height:5em;'><?php print $current->SpecialInstructions; ?></textarea></div>
   </div>
</div>
