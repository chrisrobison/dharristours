...
//data = $.parseJSON(data); 
      ...
      //                  data.rows[0]['elements'][0].City.text
			data["Street"] + ", " + data["City"] + ", " + data["School"] 
                      );
                  }
               } catch(err) { }
            }
...
//  {Select: function(id) { upAll(id); }, "New": function() { myNew(); }} 
   function myNew() {
	...
	roundtrip.options[0].selected = true;
 	$('#mygrid').trigger('reloadGrid');
      $("#travelTime").val("");
      $("#travelTime").html("");
      $("#estPrice").html("");
...
//      $("#RoundTrip").val("1");
   ...
   function updateTimeDiffinHours(who, mytime) {
      ...
      var rt = $( "#RoundTrip option:selected" ).val(); //added
      if (((rt == "0") || (rt == "false"))) //added && (jQuery("#Hours").val() == "4" )
      { //added
	jQuery("#Hours").val(2.25); //added
} else { //added
  if ((parseInt(endhrmin) - parseInt(starthrmin)) < 240) {
   ...
   function ToggleEndTime() {
      ...
      if ((rt == "1") || (rt == "true")) {
         ...
         if ($("#FinalDropOffLocation").val() == 'One Way Xfer' || $("#FinalDropOffLocation").val() == null) {
...
//         $("#RoundTrip").val("1");
  //       var pu = $("#PickupLocation").val();
    //     if ($("#FinalDropOffLocation").val() == 'One Way Xfer' || $("#FinalDropOffLocation").val() == null || $("#FinalDropOffLocation").val() == '') {
//	   $("#FinalDropOffLocation").val(pu);//added
  //       $('#mygrid').setCell(simpleConfig.rowid, "FinalDropOffLocation", $("#FinalDropOffLocation").val(), 'modified'); //added
	//	}
    //     $('#mygrid').setCell(simpleConfig.rowid, "RoundTrip", $("#RoundTrip option:selected").val(), 'modified'); //added
