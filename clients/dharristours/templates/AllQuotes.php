...
//      if (btn) {
//	var dropo = $("#DropOff").val();
//	var picku = $("#Pickup").val();
//       } else {
//	var dropo = simpleConfig.record.DropOff;
//	var picku = simpleConfig.record.Pickup;
//       }
         $.getJSON("/tools/distance.php?origin=" + encodeURI(simpleConfig.record["Pickup"]) + "&dest=" + encodeURI(simpleConfig.record.DropOff) + "&mode=driving&language=en-US&sensor=false", 
   //      $.getJSON("/tools/distance.php?origin=" + encodeURI(picku) + "&dest=" + encodeURI(dropo) + "&mode=driving&language=en-US&sensor=false", 
         function(data) {
            $("#travelTime").html( 
...
<div class='tableGroup'>
   ...
   <span id="customButtons">
      ...
      <a name='createInvoiceButton' class='simpleButton disabled' onclick='return openWin("<?php print $boss->app->Assets; ?>/templates/QuoteToJob.php?ID="+simpleConfig.id, "btnJobWin")'><span class='ui-icon ui-icon-print'></span>Gen Job</a>
      <a name='sendEmailButton' class='simpleButton disabled' onclick='sendEmail()'><span class='ui-icon ui-icon-print'></span>Send Email</a>
   </span>
   <div class='fieldcontainer'>
      <div class='fieldcolumn fieldfloater'>
         ...
         <div class='contentField'>
            <label>Status: </label>		
            <select name='Quote[<?php print $current->QuoteID; ?>][Status]' id='Status'><?php print $boss->utility->makeListOptions('QuoteStatus', ''); ?></select>
         </div>
...
<select id='End_hour' onchange="updateTime('End');">
      ...
      <div class='fieldcolumn fieldfloater'>
         <div class='contentField'><span class='fieldLabel' id='travellabel'></span><span id='travelTime' ></span></div>
         <div class='contentField'><span class='fieldLabel'>Pickup <img src='/img/map_icon.png' width='28' height='28' class='simpleButton mapButton' style='float:right;' onclick="return showMap($('#Pickup').val(), 'Pickup Address');" />: </span><input type='text' name='Quote[<?php print $current->QuoteID; ?>][Pickup]' id='Pickup' value='<?php print $current->Pickup; ?>' size='50' class='boxValue' onchange='' /></div>
         <div class='contentField'><span class='fieldLabel'>Drop <img src='/img/map_icon.png' width='28' height='28' class='simpleButton mapButton' style='float:right;' onclick="return showMap($('#DropOff').val(), 'Drop Off Address');" /></span><input type='text' name='Quote[<?php print $current->QuoteID; ?>][DropOff]' id='DropOff' value='<?php print $current->DropOff; ?>' size='50' class='boxValue' onchange='' /></div>
