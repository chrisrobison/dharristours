...
<?php print $current->Notes; ?>
      ...
      <div class='fieldcolumn'>
         ...
         <div class='contentField'><label>Check Date</label><input type='text' dbtype='varchar(50)' name='Payment[<?php print $current->PaymentID; ?>][CheckDate]' id='CheckDate' value='<?php print $current->CheckDate; ?>' size='50' class='boxValue date' /></div>
         <div class='contentField'><label>Invoice IDs</label><?php $boss->db->addResource("Invoice");$arr = $boss->db->Invoice->getlist();print $boss->utility->buildSelect($arr, $current->InvoiceID, "InvoiceID", "Invoice", "Payment[$current->PaymentID][InvoiceIDs]")."</div>";?>
         <div class='contentField'><label>Job IDs</label><?php $boss->db->addResource("Job");$arr = $boss->db->Job->getlist();print $boss->utility->buildSelect($arr, $current->JobID, "JobID", "Job", "Payment[$current->PaymentID][JobIDs]")."</div>";?>
</div>
