...
<body>
    <div id="main" class='shadow'>
        <div id='top_overview'>
            <table>
                <tr>
                    ...
                    <td>
                        <h3><?php print $current->BusinessID . '-'. date('ymd', strtotime($current->Date)) . '-' . $current->InvoiceParentID; ?>
                        </h3>
                    </td>
                </tr>
                <tr class='header'>
                    <td colspan='2' style='text-align:right;'><?php print date("m/d/Y", strtotime($current->Date)); ?></td>
                </tr>
        ...
        <div class="Part">
            ...
            ?>
                        <div contenteditable class="date" style='width:30em; border: none;font-size:18px;'>
                            <?php print $business->Address1 . "<br>" . $business->Address2 . "<br>" . $business->City; ?>,
                            <?php print $business->State; ?>. <?php print $business->Zip; ?></div>
...
print <<<EOT
      <tr>
         ...
         <td>{$job->Job}</td>
         <td>{$date}</td>
         <td class='currency'>\${$invamt}</td>
         <td class='currency'>\${$paidamt}</td>
         <td class='currency'>\${$bal}</td>
      </tr>
EOT;
                                $tot += $inv->InvoiceAmt;
                                $paid += $inv->PaidAmt;
                                ...
                                $paid = number_format($paid, 2); $tot = number_format($tot, 2);
                                ?>
                        </tbody>
                    </table>
                    <br>
                    <table style='width:7.5in;'>
                        <tr>
                            <td class='field'
                                style='border-right:1px solid #0006; color: #000; white-space:nowrap;font-weight: bold; font-size:12pt'>
                                Invoices Total:</td>
                            <td class='value right' style='width:5rem;font-weight: bold; font-size:12pt'>
                                $<?php print $tot; ?></td>
                        </tr>
                        <tr>
                            <td class='field'
        ...
        <h2 style='text-align:center;margin:0.25em 0;'>Thank you for your business!</h2>
        <div class="policies" style="font-size:9px;display:flex;">
            <ul>
                <li><b>Cancellation Policy:</b>
                        A cancellation charge of $650 per bus applied if your confirmed reservation is not cancelled 7 days prior to spot time; The full quoted amount is charged if not cancelled 72 hours prior (excluding sports tournaments). No cancellation charge due to weather if notified by 4pm the day before the confirmed trip.
                </li>
                <li><b>Late Payment:</b>
                        A 10% late fee will be added each month to any invoice 30 days past due.
                </li>
            </ul>
        </div>
    </div>
