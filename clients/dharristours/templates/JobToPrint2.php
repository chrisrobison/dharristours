<?php
   ...
   $boss->db->Job->execute("SELECT JobID, Job from Job where ((JobDate > CURDATE()) or (JobDate < (CURDATE() + INTERVAL 2 MONTH))) or JobID not in (select JobID from Invoice) OR (JobID='{$in['ID']}') order by JobDate desc");
   ...
   <body>
      <div id='toolbar'>
         <form method='post'>
            ...
            <span style='float:right;padding:0 2em;'><label>Invoice ID: </label><span class='val'><?php print $current->related_Invoice[0]->InvoiceID; ?></span></span>
