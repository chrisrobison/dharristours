...
<?php
 ...
 /* check connection */
    ...
    $sql = "select JobID, PickupLocation, DropOffLocation, FinalDropOffLocation from Job where EstDistance=0 and JobCancelled!=1 and Confirmed=1 and EstDistance='0.0' order by JobID desc";
    ...
    while ($job = mysqli_fetch_object($results)) {
        ...
        foreach ($waypoints as $waypoint) {
            ...
            if (!array_key_exists($waypoint, $seen)) {
                ...
                $www = `curl -s --include --header "Accept: application/json, application/geo+json, application/gpx+xml, img/png; charset=utf-8" 'https://nominatim.openstreetmap.org/search?q={$newwaypoint}&format=geocodejson'`;
               
                $parts = preg_split("/\r?\n\r?\n/s", $www);
                $geo = json_decode($parts[1]);
                
                $latlon = $geo->features[0]->geometry->coordinates;
                $fullname = $geo->features[0]->properties->geocoding->label;
                $name = $geo->features[0]->properties->geocoding->name;
                print_r($geo);
                $seen[$waypoint] = $geo;
