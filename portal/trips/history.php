<?php  
    ...
    if (array_key_exists("id", $in)) {
        $current = $boss->getObject("Business", $in['id']);
        $current->ContactName = $current->AttnTo;
        $current->ContactPhone = $current->Phone;
        $current->ContactEmail = $current->Email;
    }

    $allbuses = $boss->getObject("Bus", "Active=1");
    $buses = array();
    foreach ($allbuses->Bus as $b) {
        if ($b->BusID) {
            $buses[$b->BusID] = $b->BusNumber;
        }
    }
...
$jobsql = "SELECT * FROM Job where BusinessID='{$busID}'";
$jobresults = mysqli_query($link, $jobsql);
$alljobs = array();
...
while ($jobrec = mysqli_fetch_object($jobresults)) {
print_r($jobrec);
    $now = date("Y-m-d H:i:s");
    $jobdate = date("Y-m-d", strtotime($jobrec->JobDate));

    $sql = "SELECT YEAR(StartTime) Year, MONTH(StartTime) Month, COUNT(LogBookID) Trips, SUM(Distance) TotalMiles,SUM(ROUND(((EndTime - StartTime)/60)/60, 1)) TotalTime from LogBook where Vehicle='{$buses[$jobrec->BusID]}' AND StartTime>'$jobdate 00:00:00' AND EndTime<'$jobdate 23:59:59' GROUP BY YEAR(StartTime), MONTH(StartTime) WITH ROLLUP";
