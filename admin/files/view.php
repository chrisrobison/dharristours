      ...
      while ($file = readdir($dh)) {
         ...
         if ((!preg_match("/^\./", $file)) && (!preg_match("/CVS|\.bak|\.old|\.sw?/", $file))) {
            if (($done < $limit) && ($count >= $start)) {
               if (preg_match("/(jpg|png|gif|svg)$/i", $file)) {
                  ...
                  $icon = (file_exists("http://net2-dev.netoasis.net/img/mimetypes/$mime.png")) ? "http://net2-dev.netoasis.net/img/mimetypes/$mime.png" : 'http://net2-dev.netoasis.net/img/mimetypes/text.png';
               }
