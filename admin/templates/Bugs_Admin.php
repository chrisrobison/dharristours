<?php
   if ($in['sendEmail']) {
      ...
      if (file_exists($msgfile)) {
         ...
         $bug['RFCDate'] = date('r');
         $bug['URL'] = "http://mpc.simplesoftwaresf.com/index.php?pid=85&mid=10&rid=Bug&tid=".$in['BugID'];
         $rep = "\$bug[\$1]";
...
<div class='contentField'><span class='fieldLabel'>Name:</span><input type='text' name='Bug[<? print $current->BugID; ?>][Title]' id='Title' value='<?php print htmlspecialchars($current->Title, ENT_QUOTES); ?>' size='50' class='boxValue' style='width:50em;' /></div>
<div class='contentField'><span class='fieldLabel'>Description:</span><textarea name='Bug[<? print $current->BugID; ?>][Bug]' id='Bug' rows='5' cols='80' class='boxValue'><?php print htmlspecialchars($current->Bug, ENT_QUOTES); ?></textarea></div>
...
<div class='contentField'><span class='fieldLabel'>Notes:</span><textarea onchange='addDate(this);' name='Bug[<? print $current->BugID; ?>][Notes]' id='Notes' rows='12' cols='80' class='boxValue'><?php print htmlspecialchars($current->Notes, ENT_QUOTES); ?></textarea></div>
