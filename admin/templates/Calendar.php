...
<head>
	...
	<link rel="stylesheet" href="/lib/js/dijit/themes/dijit.css" type="text/css"/>
	<link rel="stylesheet" href="/lib/js/dijit/themes/tundra/tundra.css" type="text/css"/>
	<link rel="stylesheet" href="/lib/js/dojoc/dojocal/resources/Grid.css" type="text/css"/>
	<link rel="stylesheet" href="/lib/js/dojoc/dojocal/resources/Event.css" type="text/css"/>
	<script src="/lib/js/dojo/dojo.js.uncompressed.js" type="text/javascript" djconfig=""></script>
	<script src="/lib/js/dijit/dijit.js.uncompressed.js" type="text/javascript"></script>
	<!-- temporariy including all dijits for test controls -->
	<script src="/lib/js/dijit/dijit-all.js.uncompressed.js" type="text/javascript"></script>
	<script src="/lib/js/dojoc/dojocal/Grid.js" type="text/javascript"></script>
	<script src="/lib/js/dojoc/dojocal/ViewSwitcher.js" type="text/javascript"></script>
...
//				firstUserCalendar = new dojoc.dojocal.UserCalendar({id: 'firstCal', color: '#eeaaff', fontColor: '#444400'}),
         ...
         <?php
            $obj->addResource('Calendar');
            $obj->Calendar->getlist();
            $events = $obj->Calendar->Calendar;
         ?>
			firstUserCalendar.addEvents(
               ...
               <?php 
                  ...
                  foreach ($events as $event) {
					?>
               {
						startDateTime: '<?php print $event->StartDateTime; ?>',
						duration: '<?php print $event->Duration; ?>',
						summary: '<?php print $event->Calendar; ?>',
						description: '<?php print $event->Notes; ?>'
					},
               <?php
                  }
               ?>
					{
						startDateTime: dojo.date.locale.format(dojo.date.add(baseDate, 'day', 2), {selector: 'date', datePattern: 'yyyy-MM-dd'}) + 'T18:30:00Z',
						duration: 3600,
			...
			secondUserCalendar.addEvents(
				[
					{
						startDateTime: dojo.date.locale.format(dojo.date.add(baseDate, 'day', 3), {selector: 'date', datePattern: 'yyyy-MM-dd'}) + 'T14:00:00Z',
						duration: 7200,
						summary: 'Rudolph\'s hoof examination',
						description: ''
					},
					{
						startDateTime: dojo.date.locale.format(dojo.date.add(baseDate, 'day', -2), {selector: 'date', datePattern: 'yyyy-MM-dd'}) + 'T19:00:00Z',
						duration: 3600,
						summary: 'Dr. Van Icicle',
						description: ''
					},
					{
						startDateTime: dojo.date.locale.format(dojo.date.add(baseDate, 'day', 1), {selector: 'date', datePattern: 'yyyy-MM-dd'}) + 'T16:45:00Z',
						duration: 3600,
