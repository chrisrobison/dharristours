...
$in{'display'} = "#host#.#domain#" if ((!$in{'display'}) && ($in{'table'} =~ /^host/));
...
if ($in{'table'} =~ /([a-zA-Z0-9]+)_([a-zA-Z0-9]+)/i) {
   ...
   if ($in{'section'} eq "EmailDomains") {
      $dat{'domains'} = buildOptions('domain', 'domainID', ' where domain=parent ', 'domain');
   }
   if ($in{'section'} eq "Accounts") {
      $dat{'domains'} = buildOptions('domain','domainID', ' where domain=parent ','domain');
      $dat{'users'} = buildOptions('user', 'userID', " where domain='$dat{'domain'}'", 'user', 'name');
   }
   if ($in{'section'} eq "Aliases") {
      $dat{'domains'} = buildOptions('domain','domainID', ' where domain=parent ','domain');
      $dat{'aliases'} = buildOptions('alias', 'aliasID', " where domain='$dat{'domain'}'", 'alias', 'routeto');
   }   
}
