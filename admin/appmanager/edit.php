...
EOT;
      for ($f=0; $f<$fcnt; $f++) {
         if (!preg_match("/Created|LastModified|ModuleID|ProcessID|Resource|Module|Process|Pre|Post|ConditionID|OverviewHeight/", $fields[$f]->Field)) {
         ...
         } elseif (preg_match("/^(Module|Process)$/", $fields[$f]->Field)) {
            $form .= "<div class='formrow'><span class='label'>".$fields[$f]->Field."</span><input type='text' size='$showsize' name='".$fields[$f]->Field."' id='".$fields[$f]->Field."' value='".$current->{$fields[$f]->Field}."' class='data' /></div><br />\n";
         } elseif (preg_match("/ModuleID|ProcessID/", $fields[$f]->Field)) {
            ...
            $form .= "<div class='formrow'><span class='label'>".$fields[$f]->Field."</span><select name='".$fields[$f]->Field."' id='".$fields[$f]->Field."' onchange='updateFields(this.options[this.selectedIndex].value)' class='dataList'>".genTableSelect($current->{$fields[$f]->Field})."</select></div>\n";
