   ...
   if (modules && modules.length) {
      ...
      for (var m=0; m<modules.length; m++) {
         ...
         */
         ...
         if (parent) parent.appendChild(mel[mod.ModuleID]);
         if (mod.Processes.length) {
            ...
            if (parent) parent.appendChild(mc[mod.ModuleID]);
            mel[mod.ModuleID].onclick = showProcess;
...
function buildProcesses(evt, mid) {
   ...
   if (mod) {
      ...
      if (content) content.src = '/timeclock/content.php?Resource='+mod.Resource+'&type=Module&ModuleID=' + mod.ModuleID;
   }
}

function buildProcessChildren(parent, procs) {
   var cel = [ ];
   var cchild = [ ];
   var cc, xtra;
...
function showProcess(evt, mid) {
   ...
   if (mid) {
      ...
      if (mod.URL) {
         ...
         url = '/timeclock/content.php?Resource='+mod.Resource+'&type=Module&ModuleID=' + mid;
      }
...
function buildActions(pid, actions) {
   if (actions && actions.length) {
      ...
      parent.setAttribute('id', pid+'_children');
      setClass(parent, 'parent');
      parent.innerHTML = '';
      
      document.getElementById(pid).parentNode.appendChild(parent);
      var act = [ ];
...
function toggleProcess(evt, pid, tid) { 
   ...
   if (pid) {
      ...
      var url = '/timeclock/content.php?Resource='+curproc.Resource+'&type=Process&ProcessID=' + pid;
      if (tid) {
         url += '&ID='+tid;
      }
      var content = document.getElementById('content');
...
function doNew(frmID) {
   ...
   for (var i=0; i<frm.elements.length; i++) {
      try {
         ...
         } else if (frm.elements[i].type.match(/radio|checkbox/i)) {
            frm.elements[i].name = newname;
...
function updateAccess(who, amt) {
   var el = document.getElementById(who);
   
   var lvl = (parseInt(el.value) + parseInt(amt));
   var orig = el.value;
   if (el) el.value = lvl;
...
function updateTime(timediv, mydate) {
   ...
   setTimeout("updateTime('" + timediv + "', " + (mydate + 1000) + ")", 1000);
