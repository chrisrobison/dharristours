   ...
   for ($e=0; $e<count($empID); $e++) {
      ...
      if ($employee['Employee'][$id]->Employee) {
         ...
         for ($i=$start; $i<=$end; $i+=86400) {
            ...
            $x = preg_split("/:/", $punches[$yr][$mo][$dy]['in']);
            $y = preg_split("/:/", $punches[$yr][$mo][$dy]['out']);
            ...
            if ($intime && $out) {
               $sheet->write($myrow, 3, $intime, $format->hours);
               $sheet->write($myrow, 4, $out, $format->hours);
            } else {
               $sheet->writeBlank($myrow, 3, $format->hours);
               $sheet->writeBlank($myrow, 4, $format->hours);
            ...
            $sheet->writeFormula($myrow, 5, '=' . $addup1 . '-' . $addup2, $format->boldtotal);
            
            $dailies[date("Ymd", $i)][] = Spreadsheet_Excel_Writer::rowcolToCell($myrow, 5); 
         ...
         $cellid = Spreadsheet_Excel_Writer::rowcolToCell($row, 5); 
         $sheet->writeFormula($overview[$id]['row'], 1, '=' . $cellid, $format->hours);
         
         $sheet->writeBlank($row, 1, $format->date);
         $sheet->write($row, 2, 'Total: ', $format->totallabel);
         $sheet->writeBlank($row, 3, $format->total);
         $sheet->writeBlank($row, 4, $format->total);

         $addup1 = Spreadsheet_Excel_Writer::rowcolToCell($toprow, 5); 
         $addup2 = Spreadsheet_Excel_Writer::rowcolToCell($row - 1, 5); 
         
         $sheet->writeFormula($row, 5, "=SUM(".$addup1.":".$addup2.")", $format->total);
