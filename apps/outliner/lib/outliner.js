...
function addJSChild(parent, child) {
...
if (outline.edit) {
   ...
   // Enter
   keyDefs[13] = { cmd:'doEnter(evt)', shiftKey:'addSibling(evt)', ctrlKey:'return true;' };
   // Tab
...
function addSibling(evt) {
   ...
   try {
      var curObj = (ui.viewing) ? ui.viewing : ui.editing;
      ...
      if (myparent) {
         if (myparent.lastChild.id && myparent.lastChild.id.match(/^node/)) {
            var newnode = myparent.id.replace(/_children/, '') + '_' + (parseInt(myparent.lastChild.id.replace(/^node.+?_(\d+)$/, '$1')) + 1);
         } else {
            var newnode = myparent.id.replace(/_children/, '') + '_' + myparent.childNodes.length + 1; 
         }
...
function addChild(evt) {
   ...
   try {
      ...
      var myparent = getObject(ui.viewing);
      var children = getObject(ui.viewing + '_children');
      
      if (children) {
         var newnode = ui.viewing + '_' + children.childNodes.length;
      } else {
         var newnode = ui.viewing + '_' + 1;
      }
...
function addParentSibling(evt) {
   ...
   try {
      ...
      /* var parentid = ui.editing.replace(/_\d+$/, ''); */
      ...
      if (parentid) var myparent = getObject(parentid); // .previousSibling;
...
function createChildContainer(who) {
   ...
   return getObject(ui.editing+'_children');
}
...
function focusNode(node) {
   try {
      if (ui.lastfocus != ui.viewing) ui.lastfocus = ui.viewing;
      if (ui.focus) unfocus(ui.focus);
      ...
      nodeobj.className = ui.focusclass + ' focused';
      
      setTimeout("editDocument('"+nodeobj.id+"')", 250);
   } catch(e) { 
      errorHandler(e);
   }
   return false;
...
function updateDocument(cnt) {
   ...
   var showframe = getObject('showDoc');
   if (editframe && showframe) {
      if (content && content.content) {
         editframe.value = content.content;
         showframe.innerHTML = textile(content.content);
      } else {
         editframe.value = '';
         showframe.innerHTML = '';
      }
   ...
   var hdn = document.contentform;

   if (hdn) {
      hdn.node.value = (content.outliner_id) ? content.outliner_id : ui.focus;
   }
 
}

function editDocument(who) {
   if (!who) return false;
   
   var editframe = getObject('editDoc');
   if (editframe) {
      var mydoc = editframe.value;
   }

   var hdn = document.contentform;

   if (hdn) {
      hdn.content.value = mydoc;
      hdn.node.value = ui.lastfocus;
      // hdn.version.value = (content.version) ? content.version : '1.0';
      if (content && content.id) hdn.id.value = content.id;
      hdn.newnode.value = who;
      hdn.x.value = 'save';
      setTimeout("document.contentform.submit();", 250);
   }
}
...
function focusUp(evt) {
   ...
   try {
      ...
      var seek;
      if (currentNode) {
         if (currentNode.parentNode.id == 'node_children') {
            return true;
         }
         ...
         if (seek && seek.id) newnode = seek.id; 
...
function doClick(evt) {
   ...
   try {
      ...
      if (who) {
         try {
            if ((ui.over != 'tree') || (!who.id.match(/^node_/)) || (who.id=='node_children')) return true;

            var nodeItem = who;
            var clickX = (evt.offsetX) ? evt.offsetX : evt.layerX;
            clickX = clickX - nodeItem.offsetLeft;
   
            if (nodeItem.id && nodeItem.id.match(/editor|editframe/)) {
...
function liveUpdate(target, src) {
   if (textile && target && src) {
      var t = getObject(target);
      var s = getObject(src);
      if (t && s) t.innerHTML = textile(s.value);
   }
}
...
function setStatus(txt) {
   if (parent && parent.document && parent.document.getElementById) {
      var stat = parent.document.getElementById('statuscon');
...
function saveOutline() {
   try {
      ...
      var content = getObject('node_children');

      if (content) {
         frm.title.value = getObject('node_1').innerHTML;
...
function press(who) {
   var el = getObject(who);
   if (el) el.style.top = '2px';
}
...
function init() {
   /*
   ui.viewing = ui.lastfocus = ui.focus = 'node_1';
   ui.editopen = 0;
   focusNode('node_1');
   outline.edit = true;
   editDocument('node_1');
//   getObject('statuscon').style.display = 'block';
   var topchild = getObject('node_children');
   if (topchild) {
      // interrupted 
   }
   */
