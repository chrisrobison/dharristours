/*
Copyright © 2006, 2008, 2011-2012 Apple Inc.  All rights reserved.
...
// "Day/month/event view" refers to the largest size, which adds a listing of upcoming Calendar events.
//
...
function toggleView (inDoItSlow)
{
	...
	gResizeAnimation.onfinished = calViewDidChange;
...
function toggleViewAnimate ()
{
	...
	{
		setTimeout (gResizeAnimation.onfinished, gResizeAnimation.duration); // call after the last frame is drawn
		gResizeAnimation.onfinished = null;
	}
	...
	if (gCurrentView == kDayView) {
		...
		// accessibility 
		gResizeAnimation.elementMonthContent.setAttribute('aria-hidden', 'true');
		gResizeAnimation.elementEventContent.setAttribute('aria-hidden', 'true');
		buttonLabel = localizedStrings['display month view'];
		...
		// accessibility
		gResizeAnimation.elementMonthContent.setAttribute('aria-hidden', 'false');
		gResizeAnimation.elementEventContent.setAttribute('aria-hidden', 'true');
		buttonLabel = localizedStrings['display upcoming events'];
		...
		// accessibility
		gResizeAnimation.elementMonthContent.setAttribute('aria-hidden', 'false');
		gResizeAnimation.elementEventContent.setAttribute('aria-hidden', 'false');
		buttonLabel = localizedStrings['hide month view and upcoming events'];
	}
	document.getElementById('toggledisplay').setAttribute('aria-label', buttonLabel);
	
}



function calViewDidChange ()
{
...
//
{
	for (var i = 0;   i <= 6;   i++)
	{
		var el = document.getElementById("DayOfWeek-" + i);
		el.innerText = fetchIPrefNarrowDayOfWeekName(i);
		el.setAttribute('title', fetchIPrefLongMixedcaseDayOfWeekName(i)); // @title translates to the AX API as AXHelp (VO+Shift+H)
	}
}
...
//
{
	...
	else
	{
		...
		monthOfInterestMonth = inJSDate.getMonth();
		monthOfInterestYear = 1900 + inJSDate.getYear();
		
		if (monthOfInterestMonth != 0)
		{
			monthOfInterestMonth = monthOfInterestMonth - 1;
		}
		else
		{
			monthOfInterestYear = monthOfInterestYear - 1;
...
function insertAndReturnRow(el){
...
//
{
	...
	if (monthDiv.hasChildNodes()) monthDiv.innerHTML = ''; // clearing innerHTML is faster than looping through and removing childNodes
	...
	if (window.Calendar)
	{
		...
		for (i = 0;   i < 42;   i++)
		{
			...
			/*
			Note: For the sake of an accessible grid, we need rows containing gridcells, though the existing implementation used
			simple line wrapping to create the rows based on known widths. If I were creating this from scratch, I'd use a loop
			within a loop. In this case, it's less risky to retrofit the single loop by adding a row element once every 7 columns.
			Cells are then inserted into the current row, instead of directly into the month grid.
			*/
			if (i%7 == 0) { // once every 7 columns
				gridRow = insertAndReturnRow(monthDiv);
			}
			
			var dateSpan = document.createElement ("span");
			dateSpan.setAttribute('role', 'gridcell');
			...
			if (i < zeroBasedFirstDayOfWeekInOurMonth)
			{
				...
				dateSpan.setAttribute("aria-disabled", "true");
			}
			else
			{
				...
				else
				{
					...
					dateSpan.setAttribute ("aria-disabled", "false");
					dateSpan.setAttribute ("id", idFromMonthDate (ourMonth, date));
					
					// selectDate was never implemented. commenting so VO doesn't announce 'clickable'
					//dateSpan.setAttribute("onclick", "selectDate(event, this.id);");
	...
	{		
		...
		var dateToCheckDayRangeInOurMonth = new Date (1900 + inJSDate.getYear(), ourMonth, dateNumberThatIsValidInOurMonth);
...
//
{
	...
	if ((gBrowsingYear == thisYear) &&  (gBrowsingMonth == thisMonth))
	{
		...
		todayCalendarNumbersGridElement.setAttribute("aria-disabled", "false");
		todayCalendarNumbersGridElement.setAttribute("aria-selected", "true");
	}
}
...
//
{
	...
	if (returnString == undefined)		// still undefined: fallback if plugin unvailable or plugin returned null (plugin error)
	{
		...
		returnString = returnString + " " + (1900 + (parseInt(inDate.getYear()))).toString();
	}
...
//
{
	...
	var monthLabel						= document.getElementById ("monthLabel"); // text element (live region) inside the monthNameAndYearNumberElement
	var dateNumber;
	...
	// We do shrink-to-fit if needed: in some locales, this string can grow quite long:
	//
	var monthText = fetchIPrefLongMonthNameAndYearNumber (gBrowsingJSDate)
	monthLabel.innerText = monthText;
	monthLabel.setAttribute('aria-label', monthText); // this should be redundant
	...
	document.title =  [localizedStrings['WidgetDisplayName:'], dayOfWeekNameElement.innerText, dateOfMonthNumberElement.innerText, monthLabel.innerText].join(' ');
}
...
function onLoaded ()
{
	...
	//
	leftButtonImg.setAttribute ("aria-label", getLocalizedString ("Previous Month"));
	rightButtonImg.setAttribute ("aria-label", getLocalizedString ("Next Month"));
	...
	if (gCurrentView == kDayView) {
		...
		// accessibility
		monthContDiv.setAttribute('aria-hidden', 'true');
		eventContDiv.setAttribute('aria-hidden', 'true');
		buttonLabel = localizedStrings['display month view'];
		...
		// accessibility
		monthContDiv.setAttribute('aria-hidden', 'false');
		eventContDiv.setAttribute('aria-hidden', 'true');
		buttonLabel = localizedStrings['display upcoming events'];
		...
		// accessibility
		monthContDiv.setAttribute('aria-hidden', 'false');
		eventContDiv.setAttribute('aria-hidden', 'false');
		buttonLabel = localizedStrings['hide month view and upcoming events'];
	...
	// Draw things, except events (they may take time to retrieve and must be drawn from calViewDidChange):
	//
	if (gCurrentView != kDayView)
		drawGrid (gBrowsingJSDate);
	...
	document.getElementById('jumpToToday').setAttribute('aria-label', localizedStrings['display current month']);
	
	if ( window.devicePixelRatio > 1 )
		updateImagesForHiDPI();
}

function updateImagesForHiDPI()
{
	document.getElementById('Background-MiddleFill').src = "Images/background_middle@2x.png";
	document.getElementById('Static-DayCard').src = "Images/daycard@2x.png";
	document.getElementById('Static-MonthCard').src = "Images/monthcard@2x.png";
	document.getElementById('Button-Left').src = "Images/bar_left@2x.png";
	document.getElementById('Button-Right').src = "Images/bar_right@2x.png";
}
...
function initEventScrollArea ()
{
	if(!gEventScrollArea)
	{
		var pngSuffix = (window.devicePixelRatio > 1 ) ? "@2x.png":".png";
		...
		gEventScrollBar.setTrackStart ("Images/scrollbar/topcap" + pngSuffix, 9);
		gEventScrollBar.setTrackMiddle ("Images/scrollbar/mid" + pngSuffix);
		gEventScrollBar.setTrackEnd ("Images/scrollbar/bottomcap" + pngSuffix, 9);
		...
		if (window.devicePixelRatio > 1 )
		{
			gEventScrollBar._track.children[0].style["background-size"] = "17px";
			gEventScrollBar._track.children[1].style["background-size"] = "17px";
			gEventScrollBar._track.children[2].style["background-size"] = "17px";
		}
...
function drawUpcomingCalEvents ()
{
	...
	if (window.Calendar) {
		...
		if (numUpcomingCalEvents > 0) {
			...
			// No upcoming events.
			eventArea.innerHTML =
				'<div class="eventBody" id="noEvents"><a href="javascript:openCalendar();">' + getLocalizedString("There are no upcoming events in Calendar today.") + '</a></div>';
		}
...
function viewCalendarAtCurrentDate ()
{
	var script;
