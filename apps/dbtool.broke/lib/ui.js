...
function dragStart(evt, id) {
  ...
  // dragObj.elNode.zIndex = dragObj.elNode.style.zIndex;
...
function dragStop(event) {
  var pos = $(dragObj.elNode).offset();
  var tbl = $(dragObj.elNode).attr('id').replace(/^table_/, '');
  ui.state[tbl].offset = pos;
  ui.state[tbl].css = $("table_"+tbl).css();
  ui.state[tbl].zindex = ui.highZ = ui.highZ + 1; 

  storeTable(tbl);
  storeValue($(dragObj.elNode).attr('id'), pos.left + ':' + pos.top + ':' + $(".fieldContainer", dragObj.elNode).css('display') + ':' + ui.highZ + ':0');
  // Stop capturing mousemove and mouseup events.
  // dragObj.elNode.style.zIndex = 999;
  /*
  if (browser.isIE) {
...
function fillSelect(who, data, current) {
   ...
   if (sel && sel.options) {
      ...
      for (var d in data) {
         ...
         opt[opt.length] = new Option(d, data[d], selected, selected);
