<?php
   ...
   $in['path'] = preg_replace("|//|", '/', preg_replace("|".$_SERVER['PHP_SELF']."|", '', $_SERVER['PATH_TRANSLATED'])).preg_replace("/\/?\~\w+\//", '', $in['relpath']);
   $in['relpath'] = preg_replace("/\/$/", '', $in['relpath']);
   ...
   <head>
      ...
      <script type='text/javascript'>
         $(document).ready(function() {
            $(".iconLink").click(function(){window.open($(this).attr("href"));});
   ...
   if (is_dir($in['path'])) {
      ...
      while ($file = readdir($dh)) {
         ...
         if ((!preg_match("/^\./", $file)) && (!preg_match("/CVS|\.bak|\.old|\.sw?/", $file))) {
            ...
            if ($icon) {
               print "<div class='icon shadow'><a $tgt $rel href='$href' class='iconLink'><img id='$idpath' src='$icon' class='iconImage' title='$href' />";
               if (!$in['nofile']) print "<br/>$file";
               print "</div>\n";
            }
         }
      }
